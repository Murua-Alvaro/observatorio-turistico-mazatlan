...
export function Business({ year, viewMode, compare, onNavigate }: Props) {
  ...
  return <section className="dataset-page sector-page">
    ...
    {focus === 'calendar' ? <section className="analysis-block">
      ...
      <div className="planning-table"><div className="planning-table__head"><span>Mes</span><span>Aéreo</span><span>Var. anual</span><span>Cruceros</span><span>Entradas extranjeras</span><span>Presión</span></div>{planningCalendar.map((m: any) => <div className="planning-table__row" key={m.period}><strong>{monthName(m.period)}</strong><span>{integer.format(m.air)}</span><span className={m.airYoy >= 0 ? 'positive' : 'negative'}>{pct(m.airYoy, true)}</span><span>{m.cruise === null ? '—' : integer.format(m.cruise)}</span><span>{m.foreign === null ? '—' : integer.format(m.foreign)}</span><b className={`pressure pressure--${m.pressure.toLowerCase()}`}>{m.pressure}</b></div>)}</div>
    </section> : null}
